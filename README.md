...
FreeSurfer, FSL, HCP Pipelines, and Workbench are **bundled inside the pipeline folder** — you do not install them separately. The pipeline resolves all tool paths automatically from the single `PIPELINE_BASE` variable you set in `.env`.
...
├── freesurfer/                  ← bundled FreeSurfer (do not move or rename)
├── fsl/                         ← bundled FSL (do not move or rename)
├── HCPpipelines/                ← bundled HCP Pipelines (do not move or rename)
└── workbench/                   ← bundled Connectome Workbench (do not move or rename)
```

> The bundled tool folders (`freesurfer/`, `fsl/`, `HCPpipelines/`, `workbench/`) must stay in the same folder as `run_pipeline.sh`. Do not move or rename them.
...
**You don't need to manually check or install any of this yourself.** The pipeline now runs a bootstrap check automatically every time you start it — it looks for everything below, and if something is missing it prints exactly what to run, or runs it for you. This table exists so you know what's being checked and why, not because you need to act on it before running.

| Requirement | Version | Checked automatically? |
|-------------|---------|------------------------|
| Bundled tools (FreeSurfer, FSL, HCPpipelines, workbench) | bundled in download | ✓ Yes — verifies folders exist |
...
That's it. Before any data is touched, the script checks your system in this order and tells you exactly what to do for anything missing:

1. **Bundled tools** — confirms `freesurfer/`, `fsl/`, `HCPpipelines/`, `workbench/` are present (if not, your download is incomplete — re-download/re-clone)
...
You normally don't need any of this — the bootstrap check above handles it. This section exists for people who prefer to set things up by hand, or who are scripting a non-interactive deployment (e.g. CI, a cluster job submission) where prompts aren't possible.
...
If unset, all derivatives (BIDS data, FreeSurfer, MRIQC, QSIPrep, fMRIPrep, ASLPrep, logs) are written inside `PIPELINE_BASE`, same as before. Set `OUTPUT_BASE` if you want to keep the bundled tools + BIDS input on one disk and point large per-subject derivatives at a separate disk or array.
...
**Full pipeline run** — this is all you normally need. The bootstrap check runs automatically first, every time:
...
→ Nothing should "fail" silently anymore. Every run starts with the bootstrap check, which walks through bundled tools, license, Docker, Python, pip packages, and Docker images in that order, stopping with a clear numbered explanation at the first thing that's actually missing. If you see a raw bash error instead of a clean `[✗]` message, please report it — that's a gap in the bootstrap check, not expected behavior.

**Bootstrap check: "Missing folder: .../HCPpipelines"** (or freesurfer/fsl/workbench)
→ Your download or `git clone` is incomplete. These tool folders ship bundled with the repository — re-download or re-clone the full thing rather than just the `scripts/` folder.
...
→ Open `HCPpipelines/Examples/Scripts/SetUpHCPPipeline.sh` and update any paths to match the bundled tool locations inside `PIPELINE_BASE`. The preflight lists every bad path explicitly.
